...
self.addEventListener('fetch',event=>{
  ...
  if(/\/backup-catalog\.json$/.test(url.pathname)){
    event.respondWith(fetch(req,{cache:'no-store'}).catch(()=>serveLocal('./backup-catalog.json',req)));
    return;
  }
  ...
  else if(/\/rynki-master-v3132\.css$/.test(url.pathname))local='./rynki-master-v3132.css';
  else if(/\/assistant-messages\.json$/.test(url.pathname))local='./assistant-messages.json';
  else if(/\/backup-catalog\.json$/.test(url.pathname))local='./backup-catalog.json';
  if(local){
